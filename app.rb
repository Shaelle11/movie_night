...
# Sinatra route for handling snack preorders
post '/preorder' do
  snack_prices = {
    'Popcorn' => 400,
  ...
  # Calculate the total amount in kobo
  @amount = snack_prices[snack_choice] * quantity * 100

  # Prepare JSON payload for Paystack transaction initialization
  payload = {
    email: email,
    amount: @amount
  }.to_json
  ...
  begin
    # Make the POST request to Paystack API for transaction initialization
...
# Route for handling Paystack payment callback
get '/payment/callback' do
  ...
  begin
    ...
    if result["status"]
      ...
      # Log transaction only if payment is successful
